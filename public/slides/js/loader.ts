...
export async function syncFromGoogle(googleUrl: string): Promise<SyncResult> {
...
export async function loadDocument(src: string, { forceSync = false }: { forceSync?: boolean } = {}): Promise<void> {
  try {
    ...
    dom.manuscript.innerHTML = `<p style="color:#ff6b6b;font-size:24px;">載入失敗: ${message}</p>`;
    return;
  }
